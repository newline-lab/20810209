...
Before continuing, make sure you have the joint\_state\_publisher package installed. If you installed ::code::`urdf_tutorial` using apt-get, this should already be the case. If not, please update your installation to include that package (use ::code::`rosdep` to check).
...
Here we defined origin of the ::code::`base_link` with ::code::' <origin xyz="0 0 1" rpy="0 0 0"/>'
...
We can then use them with the name given, here it's "blue". In this new urdf we added also ::code::`<axis xyz="0 1 0"/>` for the **joint1** and the **joint3** this is the axis along which the links are rotating. You can change it to ::code::`<axus xyz="1 0 0"/>` to see what happens.
